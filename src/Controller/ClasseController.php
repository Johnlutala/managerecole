...
final class ClasseController extends AbstractController
{
    ...
    public function index(
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $classeExistante = $repository->findOneBy([
                'nom' => $classe->getNom(),
                'ecole' => $classe->getEcole(),
                'section' => $classe->getSection(),
            ]);
            ...
            if ($classeExistante) {
                ...
                $this->addFlash(
                    'warning',
                    'Cette classe existe déjà dans cette section.'
                );
            ...
            if ($this->getUser() instanceof User) {
                $classe->setCreatedBy($this->getUser());
            }
    ...
    public function edit(Request $request, Classe $classe, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $entityManager->flush();
            $this->addFlash('success', 'Classe mis(e) à jour avec succès.');
            return $this->redirectToRoute('app_classe_index');
        }
    ...
    public function delete(Request $request, Classe $classe, EntityManagerInterface $entityManager): Response
    {
        if ($this->isCsrfTokenValid('delete' . $classe->getId(), $request->getPayload()->getString('_token'))) {
            ...
            $entityManager->flush();
            $this->addFlash('success', 'Classe supprimé(e) avec succès.');
        }
        return $this->redirectToRoute('app_classe_index');
    }
}
